...
var lastOne = '0';
var chunk = 1000;
var myAccount = 'thepunisher';
var myMemo = 'fee refund';
...
function transferTheTips() {
   ...
   var confirm_ = confirm("This is a clientside application, which means if you close this window, the application will stop without completeing all the transactions. The first transaction will be the fee " + myFee_ + ". All transfers are non-refundable. Are you sure you want to continue?");
   ...
   if (confirm_) {
      ...
      steem.broadcast.transfer(activeKey, accountName, myAccount, myFee_, myMemo, function (err, result) {
         if (err == null) {
            appendHTML("Transfered " + myFee_ + " To NAMETOTRANSFER");
            var activeKey = document.getElementById('activeKey').value;
            var accountName = document.getElementById('accountName').value;
            var tipMessage = document.getElementById('tipMessage').value;
            var tipAmount = document.getElementById('tipAmount').value;
